...
#
# 检查点和目录设置
parser.add_argument("--logdir", type=str, default="logname_btcv")
parser.add_argument("--data_dir", type=str, default="/hy-tmp/BTCV/btcv_split_18_12/")  # 必须以分隔符"/"来结尾
parser.add_argument("--json_list", type=str, default="btcv_18_12.json")
parser.add_argument("--resume_ckpt", action="store_true")
parser.add_argument("--pretrained_dir", type=str, default="./runs/logname_btcv/")
parser.add_argument("--pretrained_model_name", type=str, default="ensemble_model_1to5.pt")
parser.add_argument("--save_checkpoint", type=bool, default=True)

# 批次大小
parser.add_argument("--max_epochs", type=int, default=3000)
parser.add_argument("--val_every", type=int, default=20)
parser.add_argument("--batch_size", type=int, default=1)  # in V100-32G GPU it's 1
...
parser.add_argument('--t_initial_multiplier', type=float, default=0.35)

# 优化器
parser.add_argument("--optimizer_name", type=str, default="AdamW")
parser.add_argument("--base_lr", type=int, default=2.5e-4)  # 根据batchsize调整 1->2.5e-4
...
parser.add_argument("--roi_x", type=int, default=120)
parser.add_argument("--roi_y", type=int, default=120)
parser.add_argument("--roi_z", type=int, default=120)
parser.add_argument("--RandFlipd_prob", type=float, default=0.4)
parser.add_argument("--RandRotate90d_prob", type=float, default=0.4)
parser.add_argument("--RandScaleIntensityd_prob", type=float, default=0.4)
parser.add_argument("--RandShiftIntensityd_prob", type=float, default=0.4)

# 模型设置
parser.add_argument("--in_channels", type=int, default=1)
parser.add_argument("--out_channels", type=int, default=14)
...
def main_worker(gpu, args):
    ...
    # 将输入张量/数组转换为离散值。
    # to_onehot：如果不是None，则将输入数据转换为具有指定数量的类的单热格式。
    post_label = AsDiscrete(to_onehot=args.out_channels)
    # argmax：是否在转换之前对输入数据执行argmax函数。
    ...
    run_training(args=args, model=model, train_loader=loader[0], val_loader=loader[1],
                 ...
                 post_label=post_label, post_pred=post_pred)  # 结束行223
    ...
    for key in state_dict_sum:
        state_dict_sum[key] /= 5
    # 将新的状态字典设置给模型
    model.load_state_dict(state_dict_sum)
    # 保存新的模型文件
    ensemble_path = os.path.join(args.logdir, 'ensemble_model_1to5.pt')
    torch.save({'state_dict': model.state_dict()}, ensemble_path)
    print("****** successful save ensemble model: ******", ensemble_path)
