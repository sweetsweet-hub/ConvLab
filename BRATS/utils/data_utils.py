...
def datafold_read(datalist, basedir, fold, key="training"):
    ...
    for d in json_data:
        tr.append(d)
        if "fold" in d and d["fold"] == fold:  # 对训练集输出或验证需注释掉，否则不注释，测试集不影响
            val.append(d)
        # else:
        #     tr.append(d)
...
def get_loader(args):  # 起始行85
    ...
    #
    #
    val_transform = transforms.Compose(  # 标准验证方法，最终结果  # 第119行
        [
            transforms.LoadImaged(keys=["image", "label"]),
            transforms.ConvertToMultiChannelBasedOnBratsClassesd(keys="label"),
            transforms.NormalizeIntensityd(keys="image", nonzero=True, channel_wise=True),
    ...
    if args.test_mode:
        datalist_json = os.path.join(data_dir, "BraTS2018_testdata.json")
